...
| **G-8** | `feasibility_decide_leads` has no owner exclusion, so a `chapter_lead` who submitted a case can record its own go/no-go | Self-decision on project intake, the analogue of the timesheet gap closed in G-1/G-7 | Decide deliberately: PF-04 names the chapter lead as decider and says nothing about their own submissions, unlike SF-2.8 for timesheets. Either accept it or add `submitted_by <> auth.uid()` to the policy |
